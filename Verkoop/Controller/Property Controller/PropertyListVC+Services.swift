    ...
    func likeCategory(params: [String: String], indexPath: IndexPath) {
        if !indices.contains(indexPath) {
            ...
            ApiManager.request(path:endPoint, parameters: params, methodType: .post, showLoader: true) { [weak self] (result) in
                ...
                if let index = self?.indices.firstIndex(where: {$0 == indexPath}) {
                    self?.indices.remove(at: index)                    
                }
                switch result {
                case .success(let data):
                    if let likeItems: LikeDislikeModel = self?.handleSuccess(data: data) {
                        let count = Int.getInt(self?.propertyData?.data?.items![indexPath.item].items_like_count) + 1
                        self?.propertyData?.data?.items![indexPath.item].items_like_count = count
                        self?.propertyData?.data?.items![indexPath.item].like_id = likeItems.like_id
                        self?.propertyData?.data?.items![indexPath.item].is_like = true
                        if let cell = self?.collectionView.cellForItem(at: indexPath) as? FilterDetailCollectionCell {
                            ...
                            cell.labelLikes.text = String.getString(self?.propertyData?.data?.items![indexPath.item].items_like_count)
                        }
                    }
                case .failure(let error):
                    self?.handleFailure(error: error)
            ...
            ApiManager.request(path:endPoint, parameters: params, methodType: .delete, showLoader: true) { [weak self](result) in
                ...
                case .success(let data):
                    if let _: LikeDislikeModel = self?.handleSuccess(data: data) {
                        self?.propertyData?.data?.items![indexPath.item].like_id = 0
                        let count = Int.getInt(self?.propertyData?.data?.items![indexPath.item].items_like_count) - 1
                        self?.propertyData?.data?.items![indexPath.item].items_like_count = count
                        self?.propertyData?.data?.items![indexPath.item].is_like = false
                        if let cell = self?.collectionView.cellForItem(at: indexPath) as? FilterDetailCollectionCell {
